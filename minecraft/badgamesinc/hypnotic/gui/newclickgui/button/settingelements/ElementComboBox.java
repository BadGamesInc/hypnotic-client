...
public class ElementComboBox extends Element {
	...
	public ElementComboBox(Button iparent, Setting iset) {
		parent = iparent;
		set = iset;
		super.setup();
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		fontRenderer.drawTotalCenteredString(setstrg, x + width / 2 + 2, y + 15/2, 0xffffffff, false);
		int clr1 = color;
		...
		if (comboextended) {
			...
			for (String sld : set.getOptions()) {
				...
				fontRenderer.drawCenteredString(elementtitle, x + width / 2 + 2, ay + 2, 0xffffffff, false);
